...
public class GraphDatabaseFacade implements GraphDatabaseAPI, EmbeddedProxySPI {
	...
	// Check if operation has any writes in it
        currentlyBatching.hasWrites = false;
	for(Find f : currentlyBatching.finds) {
	    if(f.getClass() == AddWithRelationship.class || f.getClass() == Update.class || f.getClass() == QueryDelete.class) {
		currentlyBatching.hasWrites = true;
		break;
	    }
	}
    ...
    private AtomicBoolean MLPWriteRunning = new AtomicBoolean();
    private BatchedFind MLPWriteRunner = null;
	...
	// Not ready to start anyway
	if(prop.finds.size() < BATCH_LIMIT && System.currentTimeMillis() - prop.created_at < BATCH_TIME_LIMIT) {
	    return false;
	}
	
	// Write running, wait
	if(MLPWriteRunning.get()) {
	    return false;	    
	}

	// Not oldest batch, need to wait
	for (int i = prop.operationsArrayIndex; i > serverOperations.oldest; i--) {
	    if (i == 0) {
 		break;
	    }
	    if (MLPWriteRunning.get()) {
	    ...
	    if(serverOperations.at(i) == null) {
		continue;
	    }
	    else if ( ((BatchedFind)serverOperations.at(i)).hasWrites && serverOperations.at(i).started == false ) {
		return false;
	    }
	    else {
		break;
	...
	if (prop.hasWrites) {
		...
		System.out.println("************ MLP Write Locked ************");
	    }		
	    MLPWriteRunning.set(true);
	    MLPWriteRunner = prop;
	}
	...
	for(int i = 0; i < prop.finds.size(); i++) {
	    opTimeWrapper.wait(prop.finds.get(i).operationNum, System.currentTimeMillis() - prop.finds.get(i).startTime);
	} 
	...
	// Batch completed
        if(prop.finds.size() == 0) {
            this.serverOperations.lock.lock();	
	    if(MLPWriteRunner == prop) {
		if(this.ENABLE_OPERATION_DEBUG_OUTPUT) {
		    System.out.println("************ MLP Write Unlocked ************");
		}	

		MLPWriteRunner = null;
		MLPWriteRunning.set(false);
	    }
            serverOperations.remove(prop);
            this.serverOperations.lock.unlock();
        }
    ...
    public Node newCreateNode(Label... labels) {
        KernelTransaction transaction = statementContext.getKernelTransactionBoundToThisThread(true);
        try (Statement ignore = transaction.acquireStatement()) {
            TokenWrite tokenWrite = transaction.tokenWrite();
            int[] labelIds = new int[labels.length];
            ...
            Add add = new Add(nodeId);
            added.put(nodeId, add.operationNum);
            serverOperations.add(add);

            this.opTimeWrapper.op(add.operationNum, 0);
    ...
    private boolean doPropagate() {
        BatchedFind to_propagate = serverOperations.getFirst();
